...
namespace Game{
	...
	inline void rd(void)
	{
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		{
			...
			if((ter[i][j]&2)&&col[i][j]&&col[i][j]!=color) tf[i][j]-=20;
...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline int gval(int x, int y)
	{
		...
		if(col[x][y])
		{
			ret+=400000/dis[x][y]/dis[x][y];
			ret+=tf[x][y]*1000/dis[x][y]/dis[x][y];
		}
	...
	void getsol(int x, int y)
	{
		...
		while(T>1e-4)
		{
			...
			if(tot>std::min(n*2.5, sqrt(army[color])+5)) val=-0x3f3f;
			// ERR("sum %d %d\n", sumf-sum+tot+1, val);
			// fflush(stderr);
		...
		ERR("val %d %d\n", csum, cval);
		if(csum>=0&&cval>0&&T-ctim<=10&&cval*4>=mx)
		{
			isr=1;
