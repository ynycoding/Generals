...
namespace Game1{
	...
	inline void rd(void)
	{
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		{
			...
			// if(col[i][j]&&(ter[i][j]&2)) tf[i][j]-=20;
	...
	inline void solve(void)
	{
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(col[i][j]!=color)
		{
			...
			if(col[i][j]&&dis[i][j]<=4) { cx=i, cy=j; break; }
			int val=100*(ter[i][j]==2?(25+avail/10):1)/(pf[i][j]+1);
			if(col[i][j])
			{
				// val*=2;
				if(ter[i][j]==2) val+=0x3f3f3f3f/(pf[i][j]+1);
			}
			if(vdep[i][j]<=10) val*=vdep[i][j];
			else val=-1;
		...
		getsol(tx, ty, vdis[cx][cy]+((ter[cx][cy]&2)?vdep[cx][cy]+5:0));
		tx=cx, ty=cy;
		while(1)
		{
			int d=pre[tx][ty];
...
namespace Game0{
	...
	inline void rd(void)
	{
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		{
			...
			if(!rev[i][j]) ter[i][j]=type[i][j];
			tf[i][j]=(col[i][j]==color?pf[i][j]:-pf[i][j]);
			if((ter[i][j]&2)&&col[i][j]&&col[i][j]!=color) tf[i][j]-=30;
			// if(col[i][j]&&col[i][j]!=color&&type[i][j]!=3) tf[i][j]-=army[color]/4;
		}
		// while(tt)
		// {
	...
	void getsol(int x, int y)
	{
		...
		while(T>1e-4)
		{
			...
			if(tot>std::min(30.0, sqrt(army[color])+5)) val=-0x3f3f;
			// ERR("sum %d %d\n", sumf-sum+tot+1, val);
			// fflush(stderr);
			if(val>mx) memcpy(in1, in, sizeof(in)), mx=val, rsum=sum;
		...
		ERR("val %d %d\n", csum, cval);
		if(csum>=0&&cval>0&&T-ctim<=20&&cval*10>=mx)
		{
			isr=1;
