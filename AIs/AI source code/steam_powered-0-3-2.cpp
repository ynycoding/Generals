...
namespace Game{
	...
	inline void rd(void)
	{
		...
		for(int a:{-1, 0, 1}) for(int b:{-1, 0, 1}) if(chkpos(kx+a, ky+b)&&pcol[kx+a][ky+b]&&pcol[kx+a][ky+b]!=color) ex=1;
		// for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		// {
...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline int gval(int x, int y)
	{
		...
		if(ter[x][y]==2&&!col[x][y]) return 0;
		if(col[x][y]==color) return 0;
		int ret=dis[x][y]+cad;
		ret+=tad[x][y];
		for(int a:{-1, 0, 1}) for(int b:{-1, 0, 1}) if(chkpos(x+a, y+b)&&ter[x+a][y+b]==1&&(!rev[x+a][y+b])) return ret+=100000;
		// if(col[x][y])
		// {
		...
		if(pcol[x][y]&&col[x][y]!=color&&dis[x][y]<=5) ret+=1000000;
		// if(col[x][y]) ret+=100;
		return ret;
	}
	...
	void getsol(int x, int y)
	{
		...
		while(T>1e-6)
		{
			...
			int pval=cur/pow(tot+1, 0.5);///sqrt(tot);///(sumf-sum+tot+1);
			int ptot=tot;
			tot=top;
				// ERR("T %Lf\n", T);
				// fflush(stderr);
			...
			int val=cur/(pow(tot+1, 0.5));///sqrt(tot);///(sumf-sum+tot*2+1);
		...
		cval/=(pow(std::max(0, ctot-(::T-ctim)), 0.5)+1);
		// if(csum>=0) cval/=sqrt(ctot);//(csumf-csum+ctot*2+1);
		// ERR("val %d %d\n", csum, cval);
		if(csum>=0&&cval>0&&::T-ctim<=1000&&cval*20>=mx&&ctot<=lim)
		{
			isr=1;
