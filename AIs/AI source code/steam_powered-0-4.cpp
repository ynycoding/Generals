...
namespace Game{
	...
	inline void rd(void)
	{
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		{
			if((ter[i][j]&2)&&col[i][j]&&col[i][j]!=color) tf[i][j]-=army[color]/8;
			// if(col[i][j]!=color&&ter[i][j]==0&&army[color]>500&&col[i][j]) tf[i][j]*=2, tf[i][j]-=10;
		}
		// while(tt)
		// {
		// 	--tt;
...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline int gval(int x, int y)
	{
		...
		int ret=dds[x][y]*10+cad;
		ret+=tad[x][y];
		// ret+=100*dds[x][y]*dds[x][y];
		if(col[x][y])
		{
			// ret+=100000/dds[x][y]/dds[x][y];
			ret+=1000000/dis[x][y]/dis[x][y]/dis[x][y]/dis[x][y]/dis[x][y];
		...
		if(col[x][y]) ret+=2000000/dis[x][y]/dis[x][y]/dis[x][y]/dis[x][y];
	...
	void getsol(int x, int y)
	{
		...
		while(CT--)
		{
			...
			while(T>1e-6)
			{
				...
				int pval=cur;///sqrt(tot);///(sumf-sum+tot+1);
				tot=top;
					// ERR("T %Lf\n", T);
					// fflush(stderr);
				if((rnd()%2)&&top)
				...
				int val=cur;///sqrt(tot);///(sumf-sum+tot*2+1);
				if(tot>std::min(lim*1.0, sqrt(army[color])+5)) val=-0x3f3f;
				// ERR("sum %d %d\n", sumf-sum+tot+1, val);
				// fflush(stderr);
				if(val>mx) memcpy(in1, in, sizeof(in)), mx=val, rsum=sum;
				else if(val<pval&&rnd()%100>=exp((val-pval)*10/T)*100)
				{
					cur=pcur, sum=psum, sumf=psumf;
			...
			{
				++ctot;
				if(col[i][j]!=color&&ter[i][j]==2&&col[i][j]) tf[i][j]+=std::min(30, ::T-ctim);
				csum+=tf[i][j]-1;
			...
			// ERR("val %d %d\n", csum, cval);
			if(csum>=0&&cval>0&&T-ctim<=10&&cval*4>=mx&&ctot<=lim)
			{
				isr=1;
