...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline void solve(void)
	{
		...
		gdis(kx, ky);
		{
			...
			for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(col[i][j]==color) ok[i][j]=1;
			// queue<pair<int, int> > q;
			// q.push({kx, ky});
			// ok[kx][ky]=1;
			// while(!q.empty())
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(col[i][j]!=color)
		{
			...
			int val=100*(ter[i][j]==2?(0/10):1)/(pf[i][j]+1);
			for(int d:{0, 1, 2, 3})
			{
				int nx=i+dx[d], ny=j+dy[d];
				if(chkpos(nx, ny)&&!rev[nx][ny]&&ter[nx][ny]==1) val+=1000;
			}
			if(col[i][j])
			{
				// val*=2;
		...
		getsol(tx, ty, std::max((int)pow(avail, 0.8), vdis[cx][cy]+((ter[cx][cy]&2)?vdep[cx][cy]+5:0)));
		tx=cx, ty=cy;
		while(1)
		{
			int d=pre[tx][ty];
