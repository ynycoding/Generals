...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline void solve(void)
	{
		...
		rd();
		cmt=0;
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(!rev[i][j]&&ter[i][j]==1) ++cmt;
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if((!rev[i][j]||ter[i][j]!=1)&&col[i][j]!=color)
		{
			gdis(i, j);
			...
			for(int a=1; a<=n; ++a) for(int b=1; b<=n; ++b) if(dis[a][b]>0)
			{
				if(ter[a][b]!=1)
				{
					...
					if(col[i][j]&&rdis[i][j]>0) v+=20*pf[i][j]+400000/((rdis[i][j]-1)*(rdis[i][j]-1));
					if(!col[i][j]&&!pf[i][j]) v+=100;
		...
		getsol(cx, cy, std::max(avail/2, (tkx?pf[tkx][tky]+2*n:0)));
		// ERR("T %d %d\n", T, color);
		// fflush(stderr);
		sout:;
		proc(cx, cy);
