...
namespace Game{
	...
	int army[N], land[N], ter[N][N], stamp[N][N], pcol[N][N], pf[N][N], tf[N][N], rev[N][N], cant[N][N];
	...
	inline void rd(void)
	{
		...
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		{
			if(chkvis(i, j))
			{
				...
				if(!col[i][j]||col[i][j]==color) cant[i][j]=1;
			}
			else if(pcol[i][j])
			{
...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	int cad;
	int in[N][N], in1[N][N], rdis[N][N], cur, sum, sumf, rsum, mx;
			...
			while(T>1e-5)
			{
				...
				else
				{
					...
					for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(!in[i][j]&&chkadj(i, j)&&sum+tf[i][j]-1>=0&&col[i][j]==color)
						pos[++top]={i, j};
					// for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(!in[i][j]&&((!tot&&col[i][j]==color)||chkadj(i, j))&&sum+tf[i][j]-1>=0)
					// 	pos[++top]={i, j};
	...
	int bfs(int kx, int ky, int c)
	{
		...
		while(!q.empty())
		{
			...
			{
				int nx=x+dx[d], ny=y+dy[d];
				if(chkpos(nx, ny)&&col[nx][ny]==color&&!ok[nx][ny]) q.push({nx, ny});
			}
		}
		return tot;
	}
	inline void solve(void)
	{
		int tkx=0, tky=0;
		ERR("T %d %d %d %d\n", T, color, cx, cy);
		for(int i=1; i<=n; ++i, ERR("\n")) for(int j=1; j<=n; ++j) ERR("%5d ", ((i==cy&&j==cx)?-1:1)*rval[j][i]);
		// for(int i=1; i<=n; ++i, ERR("\n")) for(int j=1; j<=n; ++j) ERR("%5d ", (ter[i][j]==1&&!rev[i][j]));
		fflush(stderr);
		rd();
		cmt=0;
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j)
		{
			if(!rev[i][j]&&ter[i][j]==1) ++cmt;
			if(pcol[i][j]&&pcol[i][j]!=color&&!cant[i][j]) ++ccf;
		}
		int ct=0, mx=-1, avail=0, tx=0, ty=0;
		ctp=0;
		memset(ok, 0, sizeof(ok));
		int ccid=0;
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(!ok[i][j]&&col[i][j]==color)
		{
			int v=bfs(i, j, ++ctp);
			if(v>mx) mx=v, ccid=ctp;
		}
		...
		gdis(kx, ky);
		memcpy(rdis, dis, sizeof(rdis));
		memset(rval, 0, sizeof(rval));
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if((!rev[i][j]||ter[i][j]!=1)&&pcol[i][j]!=color)
		{
			gdis(i, j);
			if(ter[i][j]==3) tkx=i, tky=j;
			for(int a=1; a<=n; ++a) for(int b=1; b<=n; ++b) if(dis[a][b]>0)
			{
				if(ter[a][b]!=1)
				{
					int v=0;
					if(::T>=50&&!rev[i][j]&&ter[i][j]==1) v+=5000000/cmt;
					if(ter[i][j]==3&&pf[i][j]<pf[cx][cy]) v+=10000000;
					if(ter[i][j]==2) v+=(avail>=400?avail*500:0);
					if(pcol[i][j]&&rdis[i][j]>0) v+=20*pf[i][j]+400000/((rdis[i][j]-1)*(rdis[i][j]-1));
					if(!cant[i][j]&&pcol[i][j]) v+=4000000/ccf;
					if(!pf[i][j]) v+=100;
					if(ter[i][j]==0) v+=100;
		...
		if(ct<=1&&col[cx][cy]==color&&pf[cx][cy]>1)
		{
			...
			for(int d:{0, 1, 2, 3})
			{
				...
				if(chkpos(nx, ny)&&ter[nx][ny]!=1&&(avail>=300||col[nx][ny]||ter[nx][ny]!=2)) 
				{
		...
		mx=-1;
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(col[i][j]==color&&ok[i][j]==ccid)
		{
			if(mx<rval[i][j]) mx=rval[i][j], cx=i, cy=j;
		}
		getsol(cx, cy, std::max(avail*2/3, (tkx?pf[tkx][tky]+2*n:0)));
		// ERR("T %d %d\n", T, color);
		// fflush(stderr);
		sout:;
		proc(cx, cy);
