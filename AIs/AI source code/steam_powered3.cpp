...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline void solve(void)
	{
		...
		ERR("spfa %d\n", tok);
		for(int i=1; i<=n; ++i, ERR("\n")) for(int j=1; j<=n; ++j) ERR("%5d ", vdep[1][j][i]);
		fflush(stderr);
		for(int i=1; i<=n; ++i) for(int j=1; j<=n; ++j) if(col[i][j]!=color)
		{
			...
			if(ter[i][j]==2)
			{
				...
				if(col[i][j]) val*=3;
				int mx=0x3f3f3f3f;
				for(int t=1; t<=tok; ++t) mx=std::min(mx, vdep[t][i][j]);
				val/=mx;
		...
		while(rT--||!cx)
		{
			...
			int dmn=0x3f3f3f3f;
			for(int i=1; i<=tok; ++i) dmn=std::min(mn, vdep[i][x][y]);
			// ERR("dmn %d\n", dmn);
