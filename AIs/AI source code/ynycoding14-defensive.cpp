...
namespace T{
	...
	int cnt[9], cnt1[9], ene[55][55], dds[55][55], tower[55][55], terrain[55][55], allied[55][55], dis[55][55];
	inline void rd(void)
	{
		for(int k=0;k<3;k++) {
			for(int i=0;i<n;i++) {
...
//	inline int dis(int a, int b) { return abs(a-kx)+abs(b-ky); }
	...
	inline void addene(int x, int y, int cf)
	{
		...
		{
			ene[i][j]+=1.0*(cf+50)/(dds[i][j])*(10000.0/dds[kx][ky]/dds[kx][ky]/dds[kx][ky]);
			ene[i][j]+=100000.0/dds[kx][ky]/dds[kx][ky];
		}
	}
	inline void addallied(int x, int y, int cf)
	{
		gdis(x, y);
		for(int i=0; i<n; ++i) for(int j=0; j<n; ++j)
		allied[i][j]+=10*cf/(dds[i][j]*dds[i][j]*dds[i][j]);
	}
...
//	inline int accept(int a, int b) { return a+rand()%(cnt[color]/100+1)>b+rand()%(cnt[color]/100+1); }
	...
	inline void procallied(void)
	{
		...
		else if(!col[i][j]&&type[i][j]==0&&!force[i][j]) addallied(i, j, 100);
	}
...
//			ret*=2;
	...
	{
		int d=-1, c=-100, cf=force[x][y];
		for(int i=0; i<4; ++i) if(gval(x+dx[i], y+dy[i], cf)>c) c=gval(x+dx[i], y+dy[i], cf), d=i;
		return d;
	}
	inline ll gmval(int x, int y)
	{
		int c=-0x3f3f3f3f, cf=force[x][y];
		for(int i=0; i<4; ++i) if(gval(x+dx[i], y+dy[i], cf)>c) c=gval(x+dx[i], y+dy[i], cf);
		return 100*cf+c;
		return c<0?c:1ll*c*cf*cf;
	}
	inline void findmax(void)
	{
		...
		ll cur;
		for(int i=0; i<n; ++i) for(int j=0; j<n; ++j)
		if(col[i][j]==color&&(cur=gmval(i, j))>c) cx=i, cy=j, c=cur;
